...
app.post('/api/productos', async (request, response) => {
    ...
    const { nombre, stock, descripcion } = request.body
    const results = await collection.insertOne({ nombre, stock, descripcion });
...
app.put('/api/productos/:id', async (request, response) => {
    ...
    const { id } = request.params
    const { nombre, stock, descripcion } = request.body
    const results = await collection.updateOne({ _id: new ObjectId(id) }, { $set: { nombre, stock, descripcion } });
...
app.listen(PORT, () => console.log(`SIUUUUUUUUUUUUUUUUUUUUUUU ${PORT}`))
